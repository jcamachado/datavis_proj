...
class PreviewServer {
  ...
  static async start({ verbose = true, hostname, port, open, ...options }) {
    ...
    if (verbose) {
      console.log(`${green(bold("Observable Framework"))} ${faint(`v${"1.6.0"}`)}`);
...
function transpileCode({ id, node }, resolvers) {
...
function diffFiles(oldFiles, newFiles) {
  ...
  for (const [name, path] of newFiles) {
    if (oldFiles.get(name) !== path) {
      patch.added.push({ name, mimeType: mime.getType(name) ?? void 0, path });
    }
  }
  return patch;
}
