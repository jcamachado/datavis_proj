...
async function deploy({ config, message, force, deployPollInterval = DEPLOY_POLL_INTERVAL_MS, maxConcurrency }, effects = defaultEffects) {
  const { clack: clack2 } = effects;
  Telemetry.record({ event: "deploy", step: "start", force });
  clack2.intro(`${inverse(" observable deploy ")} ${faint(`v${"1.6.0"}`)}`);
  ...
  if (!doBuild && !force && effects.isTty) {
    ...
    if (mostRecentSourceMtimeMs > leastRecentBuildMtimeMs) {
      message2 += "Your source files have changed since the last time you built. ";
      initialValue = true;
    }
    message2 += `You built this project ${formatAge(buildAge)}. `;
    message2 += "Would you like to build again before deploying?";
    const choice = await clack2.confirm({
      message: message2,
      initialValue,
      active: "Yes, build and then deploy",
...
async function findMostRecentSourceMtimeMs(effects, config) {
  ...
  for await (const file of effects.visitFiles(config.root)) {
    ...
    const stat2 = await effects.stat(joinedPath);
...
async function findLeastRecentBuildMtimeMs(effects, config) {
