...
function normalizePages(spec) {
  return Array.from(
    spec,
    (spec2) => "pages" in spec2 ? normalizeSection(spec2, (spec3) => normalizePage(spec3)) : normalizePage(spec2)
  );
}
function normalizeSection(spec, normalizePage2) {
  let { name, open, collapsible = open === void 0 ? false : true, pages } = spec;
  name = String(name);
  collapsible = Boolean(collapsible);
  open = collapsible ? Boolean(open) : true;
  pages = Array.from(pages, normalizePage2);
